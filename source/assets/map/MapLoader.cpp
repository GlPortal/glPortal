...
void MapLoader::extractSpawn() {
  ...
  spawnElement = rootHandle.FirstChild("spawn").ToElement();
...
void MapLoader::extractLights() {
  ...
  lightElement = rootHandle.FirstChild("light").ToElement();
...
void MapLoader::extractDoor() {
  ...
  endElement = rootHandle.FirstChild("end").ToElement();
    ...
    do {
      ...
      if (wallBoxElement) {
        do {
          ...
          TiXmlElement *rotElement = wallBoxElement->FirstChildElement("rotation");
          if (rotElement) XmlHelper::pushAttributeVertexToVector(rotElement, wall.rotation);
