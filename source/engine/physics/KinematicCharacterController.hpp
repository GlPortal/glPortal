...
This file is taken from the original Bullet engine source, and is MODIFIED to add support for what
we need.
...
/// with discrete collision detection to recover from penetrations.
/// Interaction between btKinematicCharacterController and dynamic rigid bodies needs to be
/// explicity implemented by the user.
...
protected:
  ...
  btConvexShape* m_convexShape;// is also in m_ghostObject, but it needs to be convex, so we store
                              // it here to avoid upcast
  ...
  btScalar m_maxSlopeCosine;  // Cosine equivalent of m_maxSlopeRadians(calculated once when set,
                              // for optimization)
  btScalar m_gravity;
