...
void Portal::placeWrapperPiece(const Vector3f &p, const Quaternion &o, const Vector3f &s,
  ...
  if (side.body) {
    entity.manager.world.getSystem<PhysicsSystem>().getPhysWorld().removeRigidBody(side.body.get());
  }
  side.body.reset(new btRigidBody(ci));
  entity.manager.world.getSystem<PhysicsSystem>().getPhysWorld().addRigidBody(side.body.get());
}
